...
function checkOberbooking(
  ...
  const { date, isReserved, isPeriod, isPast } = day;

  if(!isOwner){
    if (isPast) return PAST;

    if (!isPeriod) return NOT_AVAILABLE;
...
const Cell = memo((props: GridChildComponentProps): JSX.Element => {
  ...
  if (day.isPeriod) day.classNames.push(styles.period);
  ...
  const periodDate = day.periodStart || day.periodEnd

  if (day.isPeriod) footer.text = titles.periodFooter;
  if (day.isReserved) footer.text = titles.reservedFooter;

  if(periodDate) footer.text = format(periodDate, "HH:mm")
...
function BookingCalendar({
  ...
  const items = useMemo(
    ...
    [startMonth, numOfMonths, reserved, period,  titles, dateFnsOptions]
  );
