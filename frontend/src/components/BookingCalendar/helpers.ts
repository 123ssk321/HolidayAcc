...
import { getPeriodInfoOfDate } from "./utils/getPeriodInfoOfDate";
...
export const createDays = ({
  ...
  for (let i in Array.from({ length: numOfMonths })) {
    ...
    while (day <= weekEnd) {
      ...
      const periodStart = getPeriodInfoOfDate(startOfDay(cloneDay), period, true);
      const periodEnd = getPeriodInfoOfDate(endOfDay(cloneDay), period);
      const isPeriod = !!period.find(
        (d) =>
          isBetween(endOfDay(cloneDay), d.startDate, d.endDate, "[]") &&
          isBetween(startOfDay(cloneDay), d.startDate, d.endDate, "[]")
      );
      ...
      const dayObj = {
        ...
        isReserved,
        periodStart: periodStart.period ? periodStart.startDate : null,
        periodEnd: periodEnd.period ? periodEnd.endDate : null,
        isPeriod,
        isPast,
        isToday,
      };
